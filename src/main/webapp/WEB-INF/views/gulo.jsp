...
<div id="shell">
    ...
    <div id="sub-navigation">
      ...
      <div id="search">
		...
		<c:choose>
			...
			<c:forEach begin="${searchVo.pageStart }" end="${searchVo.pageEnd }" var="i"></c:forEach>
          <input type="text" name="sk" value="${resk }" id="search-field" class="blink search-field" 
          	placeholder="지역 및 식당이름" 	onfocus="this.placeholder=''" onblur="this.placeholder='지역 및 식당이름'"  />
  ...
  <div id="main">
	...
	var positions = [
    	...
    	{
    		title: '<c:out value="${gulo.sikname }"/>',
    		address: '<c:out value="${gulo.sikaddress }"/>'
    	},
		</c:forEach>
    ];
...
현재페이지/토탈페이지 : ${searchVo.page }/${searchVo.totPage } <br /> --%>
	...
	<c:if test="${searchVo.page>1 }">
		<a href="gulo?page=1&com=구로">[처음]</a>
		<a href="gulo?page=${searchVo.page-1 }&com=구로">[이전]</a>
	</c:if>
	...
	<c:forEach begin="${searchVo.pageStart }" end="${searchVo.pageEnd }" var="i">
		<c:choose>
			...
			<c:otherwise>
			<a href="gulo?page=${i }&sikloca=${sikloca }&sikcon=${sikname }&sk=${resk }&com=구로" 
	...
	<c:if test="${searchVo.page<searchVo.totPage }">
		<a href="gulo?page=${searchVo.page+1 }&com=구로">[다음]</a>
		<a href="gulo?page=${searchVo.totPage }&com=구로">[마지막]</a>
	</c:if>
	</form>
